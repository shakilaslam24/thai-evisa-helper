...
function partnerDashboard(req, res) {
  ...
  const byStatus = Object.fromEntries(
    db.prepare('SELECT status, COUNT(*) n FROM case_files WHERE deleted_at IS NULL AND partner_id = ? GROUP BY status')
      .all(pid).map((r) => [r.status, r.n])
  );
  const money = db.prepare(`SELECT COALESCE(SUM(total),0) billed, COALESCE(SUM(paid),0) collected,
    COUNT(*) invoices FROM invoices WHERE deleted_at IS NULL AND partner_id = ? AND status != 'Cancelled'`).get(pid);
  const thisMonth = db.prepare(`SELECT COALESCE(SUM(total),0) billed, COALESCE(SUM(paid),0) collected,
    COUNT(*) invoices FROM invoices WHERE deleted_at IS NULL AND partner_id = ? AND status != 'Cancelled'
    AND strftime('%Y-%m', issue_date) = strftime('%Y-%m','now','localtime')`).get(pid);

  const stats = {
    total_files: count('SELECT COUNT(*) n FROM case_files WHERE deleted_at IS NULL AND partner_id = ?', pid),
    active_files: count(
      `SELECT COUNT(*) n FROM case_files WHERE deleted_at IS NULL AND partner_id = ? AND status IN (${ACTIVE_PLACEHOLDERS})`,
    ...
    unpaid_invoices: count(
      "SELECT COUNT(*) n FROM invoices WHERE deleted_at IS NULL AND partner_id = ? AND status IN ('Unpaid','Partial Paid')", pid),
    ...
    total_customers: count('SELECT COUNT(*) n FROM customers WHERE deleted_at IS NULL AND partner_id = ?', pid),
  ...
  const monthlySales = db.prepare(`
    ...
    FROM invoices WHERE deleted_at IS NULL AND partner_id = ? AND status != 'Cancelled'
  ...
  const upcomingInterviews = db.prepare(`
    ...
    WHERE f.deleted_at IS NULL AND f.partner_id = ? AND f.interview_date IS NOT NULL
...
router.get('/', wrap((req, res) => {
  ...
  const fileCounts = Object.fromEntries(
    db.prepare(`SELECT status, COUNT(*) n FROM case_files WHERE deleted_at IS NULL${byMe('assigned_to')} GROUP BY status`)
      .all().map((r) => [r.status, r.n])
  );

  const stats = {
    total_leads: count(`SELECT COUNT(*) n FROM leads WHERE deleted_at IS NULL${byMe('assigned_to')}`),
    new_leads: count(`SELECT COUNT(*) n FROM leads WHERE deleted_at IS NULL AND status = 'New Lead'${byMe('assigned_to')}`),
    leads_today: count(`SELECT COUNT(*) n FROM leads WHERE deleted_at IS NULL
      AND date(created_at) = date('now','localtime')${byMe('assigned_to')}`),
    converted_leads: count(`SELECT COUNT(*) n FROM leads WHERE deleted_at IS NULL AND status = 'Converted'${byMe('assigned_to')}`),
    ...
    total_customers: count(`SELECT COUNT(*) n FROM customers WHERE deleted_at IS NULL${byMe('assigned_to')}`),
    total_files: count(`SELECT COUNT(*) n FROM case_files WHERE deleted_at IS NULL${byMe('assigned_to')}`),
    active_files: count(
      `SELECT COUNT(*) n FROM case_files WHERE deleted_at IS NULL
       AND status IN (${ACTIVE_PLACEHOLDERS})${byMe('assigned_to')}`,
    ...
    total_partners: count('SELECT COUNT(*) n FROM partners WHERE deleted_at IS NULL'),
    active_partners: count("SELECT COUNT(*) n FROM partners WHERE deleted_at IS NULL AND status = 'Active'"),

    total_invoices: count(`SELECT COUNT(*) n ${MONEY_FROM} WHERE i.deleted_at IS NULL AND i.status != 'Cancelled'${mySales}`),
  ...
  const money = db.prepare(`
    ...
    ${MONEY_FROM} WHERE i.deleted_at IS NULL AND i.status != 'Cancelled'${mySales}
  `).get();
  ...
  const leadTrend = db.prepare(`
    ...
    WHERE deleted_at IS NULL
      AND created_at >= date('now','localtime','-11 months','start of month')${byMe('assigned_to')}
    GROUP BY month ORDER BY month
  `).all();
  ...
  const filesByStatus = db.prepare(
    `SELECT status, COUNT(*) n FROM case_files WHERE deleted_at IS NULL${byMe('assigned_to')}
...
router.get('/search', denyPartner, wrap((req, res) => {
  ...
  for (const row of db.prepare(`
    ...
    WHERE deleted_at IS NULL
      AND (full_name LIKE ? OR phone LIKE ? OR whatsapp LIKE ? OR email LIKE ?) LIMIT 8
  ...
  for (const row of db.prepare(`
    ...
    WHERE deleted_at IS NULL
      AND (given_name LIKE ? OR surname LIKE ? OR passport_no LIKE ? OR phone LIKE ? OR email LIKE ?) LIMIT 8
  ...
  for (const row of db.prepare(`
    ...
    WHERE f.deleted_at IS NULL
      AND (f.reference_no LIKE ? OR c.passport_no LIKE ? OR c.given_name LIKE ? OR c.surname LIKE ?) LIMIT 8
  `).all(like, like, like, like)) {
  ...
  for (const row of db.prepare(`
    ...
    WHERE deleted_at IS NULL
      AND (partner_name LIKE ? OR company_name LIKE ? OR email LIKE ?
  ...
  for (const row of db.prepare(`
    SELECT id, invoice_no, total, status FROM invoices
    WHERE deleted_at IS NULL AND invoice_no LIKE ? LIMIT 5
  `).all(like)) {
