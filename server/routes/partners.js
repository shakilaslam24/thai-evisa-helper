...
const {
  wrap, notFound, pick, requireFields, oneOf, paging, orderBy, conditions,
...
const SELECT = `
  SELECT p.*, u.name AS created_by_name,
    (SELECT COUNT(*) FROM case_files f WHERE f.partner_id = p.id) AS total_files,
    (SELECT COUNT(*) FROM case_files f WHERE f.partner_id = p.id AND f.status = 'Approved') AS approved_files,
    (SELECT COUNT(*) FROM case_files f WHERE f.partner_id = p.id AND f.status = 'Rejected') AS rejected_files,
    (SELECT COALESCE(SUM(i.total - i.paid), 0) FROM invoices i
      WHERE i.partner_id = p.id AND i.status != 'Cancelled') AS outstanding_due
  FROM partners p
...
router.get('/:id', wrap((req, res) => {
  ...
  const byStatus = db.prepare(`SELECT status, COUNT(*) n FROM case_files
                               WHERE partner_id = ? GROUP BY status`).all(id);
  const counts = Object.fromEntries(byStatus.map((r) => [r.status, r.n]));
  const processing = db.prepare(
    `SELECT COUNT(*) n FROM case_files WHERE partner_id = ? AND status IN (${ACTIVE_LIST})`
  ...
  const pendingDocs = db.prepare(`
    ...
    WHERE f.partner_id = ? AND dc.status = 'Missing'
  `).get(id).n;
...
router.get('/:id/invoices', wrap((req, res) => {
  const id = Number(req.params.id);
  guardPartnerScope(req, id);
  const rows = db.prepare(`SELECT * FROM invoices WHERE partner_id = ? ORDER BY issue_date DESC, id DESC`).all(id);
  res.json({ data: rows });
}));
...
router.post('/', canWrite('partners'), wrap((req, res) => {
  requireFields(req.body, ['partner_name']);
...
router.delete('/:id', canWrite('partners'), wrap((req, res) => {
  ...
  logActivity('partner', partner.id, 'Partner deleted', partner.partner_name, req.user.id);
  res.json({ ok: true });
}));
