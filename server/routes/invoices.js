...
const SELECT = `
  SELECT i.*, trim(c.given_name || ' ' || COALESCE(c.surname,'')) AS customer_name,
         ...
         f.reference_no, u.name AS created_by_name,
...
/** Invoice numbers use the configurable prefix and never collide. */
function nextInvoiceNo() {
  ...
  const seq = last ? Number(String(last.invoice_no).slice(prefix.length)) + 1 : 1;
...
function recalcInvoice(invoiceId) {
  ...
  db.prepare("UPDATE invoices SET paid = ?, status = ?, updated_at = datetime('now') WHERE id = ?")
  ...
  if (invoice.case_file_id) {
    const roll = db.prepare(`SELECT COALESCE(SUM(total),0) t, COALESCE(SUM(paid),0) p
                             FROM invoices WHERE case_file_id = ? AND status != 'Cancelled'`)
      .get(invoice.case_file_id);
    db.prepare("UPDATE case_files SET payment_status = ?, updated_at = datetime('now') WHERE id = ?")
...
router.post('/', canWrite('invoices'), wrap((req, res) => {
  ...
  const create = db.transaction(() => {
    ...
    `).run({
      invoice_no: clean(b.invoice_no) || nextInvoiceNo(),
      ...
      issue_date: clean(b.issue_date) || todayISO(),
  ...
  const update = db.transaction(() => {
    db.prepare(`
      UPDATE invoices SET customer_id=@customer_id, partner_id=@partner_id, case_file_id=@case_file_id,
        ...
        discount=@discount, tax=@tax, total=@total, notes=@notes, updated_at=datetime('now')
      WHERE id=@id
    `).run({
      customer_id: b.customer_id ? Number(b.customer_id) : null,
      partner_id: b.partner_id ? Number(b.partner_id) : null,
      case_file_id: b.case_file_id ? Number(b.case_file_id) : null,
      issue_date: clean(b.issue_date) || before.issue_date,
      due_date: clean(b.due_date),
      currency: clean(b.currency) || before.currency,
      subtotal, discount, tax, total, notes: clean(b.notes), id,
    });
...
router.post('/:id/cancel', canWrite('invoices'), wrap((req, res) => {
  ...
  if (!invoice) notFound('Invoice not found');
  if (invoice.paid > 0) bad('This invoice has payments recorded — delete the payments first');
...
router.delete('/:id', canWrite('invoices'), wrap((req, res) => {
  const invoice = db.prepare('SELECT * FROM invoices WHERE id = ?').get(Number(req.params.id));
  if (!invoice) notFound('Invoice not found');
  const fileId = invoice.case_file_id;
  db.prepare('DELETE FROM invoices WHERE id = ?').run(invoice.id);
  if (fileId) {
    const roll = db.prepare(`SELECT COALESCE(SUM(total),0) t, COALESCE(SUM(paid),0) p
                             FROM invoices WHERE case_file_id = ? AND status != 'Cancelled'`).get(fileId);
    db.prepare('UPDATE case_files SET payment_status = ? WHERE id = ?')
      .run(roll.t > 0 ? statusFor(roll.t, roll.p) : 'Unpaid', fileId);
  }
  logActivity('invoice', invoice.id, 'Invoice deleted', invoice.invoice_no, req.user.id);
  res.json({ ok: true });
}));
